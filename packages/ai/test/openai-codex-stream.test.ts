...
describe("openai-codex streaming", () => {
	...
	// C2 dispatch contract: the limit error arrives after socket.send(), so the
	// send was performed. Reconnecting would be a second inference-bearing send
	// without fresh permission — the outcome is terminal uncertainty instead.
	it("stops uncertain when the websocket connection limit error arrives post-send", async () => {
		const token = mockToken();
		let connections = 0;
		...
		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toContain("websocket_connection_limit_reached");
		expect(connections).toBe(1);
		expect(fetchMock).not.toHaveBeenCalled();
	});

	// C2 dispatch contract: the send was already performed when the idle timer
	// fires, so falling back to SSE would silently resend. Terminal instead.
	it("stops uncertain when a websocket goes idle after send (no SSE resend)", async () => {
		vi.useFakeTimers();
		const token = mockToken();
		const sentBodies: unknown[] = [];
		const encoder = new TextEncoder();
	...
	// C2 dispatch contract: the delta send was performed before the
	// previous_response_not_found error arrived, so neither a websocket resend
	// nor an SSE fallback may run — the outcome is terminal uncertainty.
	it("stops uncertain on a missing cached websocket continuation (post-send, no resend)", async () => {
		const token = mockToken();
		...
		expect(second.errorMessage).toContain("not found");
		expect(eventTypes).toContain("error");
		expect(connections).toBe(1);
		expect(sentBodies).toHaveLength(2);
		expect(sentBodies.map((body) => body.connectionId)).toEqual([1, 1]);
	...
	// C2 dispatch contract positive side: a fetch that throws a provably
	// pre-send connection error never reached the server, so the retry branch
	// is preserved for it.
	it("retries a provably pre-send SSE connection failure", async () => {
		vi.useFakeTimers();
		const token = mockToken();
		const encoder = new TextEncoder();
		const sse = buildSSEPayload({ status: "completed" });
		...
		const resultPromise = streamOpenAICodexResponses(model, normalizeContext({ systemPrompt: "", messages: [] }), {
			apiKey: token,
			transport: "sse",
			maxRetries: 1,
		}).result();
	...
	// C2 dispatch contract: a received error status means the send was
	// performed, so retry-after headers are never honored for an automatic
	// resend — the stream ends uncertain after the single send.
	it.each([
	...
	] as const)("post-send 429 with %s is terminal (no automatic resend)", async (_name, makeHeaders, expectedDelay) => {
		vi.useFakeTimers();
		...
		await vi.advanceTimersByTimeAsync(0);
		expect(setTimeoutSpy).not.toHaveBeenCalledWith(expect.any(Function), expectedDelay);

		const result = await resultPromise;
		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toBe("You have hit your ChatGPT usage limit.");
		expect(codexRequests).toBe(1);
	});

	// C2 dispatch contract: any post-send error status is terminal — the
	// retry-delay cap is moot because no resend is attempted at all.
	it.each([429, 503])("post-send %i is terminal regardless of retry-after headers", async (status) => {
		const token = mockToken();
		const fetchMock = vi.fn(
			async () =>
		...
		expect(result.stopReason).toBe("error");
		expect(result.errorMessage).toBe(status === 429 ? "You have hit your ChatGPT usage limit." : "retry later");
		expect(fetchMock).toHaveBeenCalledTimes(1);
	});
	...
	// C2 dispatch contract: the first post-send 429 ends the stream; no
	// backoff resend is ever scheduled.
	it("post-send 429 is terminal — no backoff resend", async () => {
		vi.useFakeTimers();
		vi.setSystemTime(new Date("2026-05-13T00:00:00Z"));
		const setTimeoutSpy = vi.spyOn(globalThis, "setTimeout");
		const token = mockToken();
		...
		await vi.advanceTimersByTimeAsync(0);
		expect(retryTimeoutDelays()).toEqual([]);
